...
IMAGE lectureImage(const char *in)
{
	...
	else
	{
		...
		if (strcmp(type, "P2") == 0)  /* cas ASCII niveaux de gris */
		{
			...
			(void)fscanf(F, "%c", &type[0]);
			(void)fscanf(F, "%c", &type[1]);
			(void)fscanf(F, "%c", &buf); /* caractère espacement */
...
SIGNATURE_COMPOSANTE_CONNEXE* signaturesImage(IMAGE label, int nbComp)
{
	...
	for (int cc = 0; cc <= nbComp; cc++)
	{
		sig[cc].HGi = label.Nblig - 1;
		sig[cc].HGj = label.Nbcol - 1;
		sig[cc].image.data = NULL;
		sig[cc].image.pixel = NULL;
	}

	for (i = 0; i < label.Nblig; i++)
		for (j = 0; j < label.Nbcol; j++)
		{
			if (i <= sig[label.pixel[i][j]].HGi) sig[label.pixel[i][j]].HGi = i;
			if (i >= sig[label.pixel[i][j]].BDi) sig[label.pixel[i][j]].BDi = i;
			if (j <= sig[label.pixel[i][j]].HGj) sig[label.pixel[i][j]].HGj = j;
			if (j >= sig[label.pixel[i][j]].BDj) sig[label.pixel[i][j]].BDj = j;
		}

	for (i = 0; i < label.Nblig; i++)
		for (j = 0; j < label.Nbcol; j++)
		{
			sig[label.pixel[i][j]].surface += 1;
			sig[label.pixel[i][j]].CGi += (float)i;
			sig[label.pixel[i][j]].CGj += (float)j;
		}

	for (int cc = 0; cc <= nbComp; cc++)
	{
		sig[cc].CGi /= sig[cc].surface;
		sig[cc].CGj /= sig[cc].surface;
		sig[cc].rayon = (float)sqrt((float)sig[cc].surface / acos(-1));
	}
	...
	/* allocations Image */
	for (int cc = 0; cc <= nbComp; cc++)
		sig[cc].image = allocationImage(sig[cc].BDi - sig[cc].HGi + 1, sig[cc].BDj - sig[cc].HGj + 1);

	/* remplissage */
	for (i = 0; i < label.Nblig; i++)
		for (j = 0; j < label.Nbcol; j++)
		{
			int etiq = label.pixel[i][j];
			sig[etiq].image.pixel[i - sig[etiq].HGi][j - sig[etiq].HGj] = 1;
		}
...
IMAGE filtrageMoyenImage(IMAGE img, int N)
{
	...
	for (int i = 0; i < img.Nblig; i++)
		...
		{
			int dk = max(0, i - N / 2);
			int fk = min(img.Nblig - 1, i + N / 2);
			int dl = max(0, j - N / 2);
			int fl = min(img.Nbcol - 1, j + N / 2);
			int nbPix = 0;
			float somme = 0;
			for (int k = dk; k <= fk; k++)
				for (int l = dl; l <= fl; l++)
			...
			somme /= nbPix;
			out.pixel[i][j] = (unsigned char)somme;
		}
...
IMAGERGB filtrageMoyenImageRGB(IMAGERGB img, int N)
{
	...
	IMAGE pR = { 0,0,NULL,NULL }, pRF = { 0,0,NULL,NULL };
	IMAGE pG = { 0,0,NULL,NULL }, pGF = { 0,0,NULL,NULL };
	IMAGE pB = { 0,0,NULL,NULL }, pBF = { 0,0,NULL,NULL };

	pR = planImage(img, 0);
	pRF = filtrageMoyenImage(pR, N);
	pG = planImage(img, 1);
	pGF = filtrageMoyenImage(pG, N);
	pB = planImage(img, 2);
	pBF = filtrageMoyenImage(pB, N);

	out = allocationImageRGB(img.Nblig, img.Nbcol);
	for (int i = 0; i < img.Nblig*img.Nbcol; i++)
	{
		out.data[i].R = pRF.data[i];
		out.data[i].G = pGF.data[i];
		out.data[i].B = pBF.data[i];
	}

	liberationImage(&pR); liberationImage(&pRF);
	liberationImage(&pG); liberationImage(&pGF);
	liberationImage(&pB); liberationImage(&pBF);
...
IMAGE filtrageBordImage(IMAGE img, int nbComp)
{
	...
	SIGNATURE_COMPOSANTE_CONNEXE* sig = signaturesImage(img, nbComp);
	int* lut = NULL;
	lut = (int*)calloc(nbComp+1, sizeof(int));
	for (int kk = 1; kk <= nbComp; kk++) {
		if (sig[kk].bord)
			lut[kk] = 0;
		else
			lut[kk] = kk;
...
void sauvegardeSignaturesImage(SIGNATURE_COMPOSANTE_CONNEXE* tab, int nbComp, const char* nomFichier)
{
	...
	fprintf(F, "cc; surface; perimetre; compacite; bord; CGi; CGj; HGi; HGj; BDi; BDj; rayon\n");
	for (int i = 0; i <= nbComp; i++)
		fprintf(F, "%d; %d; %.2f; %.2f; %d; %.2f; %.2f; %d; %d; %d; %d; %.2f\n", i, tab[i].surface, tab[i].perimetre, tab[i].compacite, tab[i].bord, tab[i].CGi, tab[i].CGj, tab[i].HGi, tab[i].HGj, tab[i].BDi, tab[i].BDj, tab[i].rayon);
...
IMAGE bouchageTrousImage(IMAGE img)
{
	...
	IMAGE BW2 = allocationImage(img.Nblig, img.Nbcol); 
	/* complément image binaire initiale */
	for (int pix = 0; pix < img.Nblig * img.Nbcol; pix++)
		if (img.data[pix] == 0)
			BW2.data[pix] = 1;
		else
			BW2.data[pix] = 0;
	...
	int nb = 0;
	IMAGE lab = labelImage(BW2, &nb);

	int* lut = calloc(nb + 1,sizeof(int));
	
	/* recherche des composantes touchant le bord */
	...
	for (int pix = 0; pix < lab.Nbcol * lab.Nblig; pix++)
		out.data[pix] = img.data[pix] + lut[lab.data[pix]];

	free(lut);
	liberationSignaturesImage(&sig,nb);
	liberationImage(&lab);
	liberationImage(&BW2);
...
IMAGE bruitAleatoireImage(IMAGE img, int amplitude)
{
	...
	for (int pix = 0; pix < img.Nblig * img.Nbcol; pix++)
	{
		...
		int test = img.data[pix] + bruit;
		if (test < 0)
			test = 0;
		else
			if (test > 255)
				test = 255;
		out.data[pix] = (unsigned char)test;
	}
...
IMAGERGB masqueImage(IMAGE img, IMAGERGB masque)
{
	...
	for (int pix=0;pix<img.Nblig*img.Nbcol;pix++)
		...
		{
			out.data[pix].R = img.data[pix];
			out.data[pix].G = img.data[pix];
			out.data[pix].B = img.data[pix];
		}
		else
		{
			out.data[pix].R = masque.data[pix].R;
			out.data[pix].G = masque.data[pix].G;
			out.data[pix].B = masque.data[pix].B;
		}
...
{
	float indiceU = 0, indiceI = 0;

	for (int i = 0; i < BW1.Nbcol * BW1.Nbcol; i++)
	{
		if ((BW1.data[i] != 0) && (BW2.data[i] != 0))
			indiceI += 1;
		if ((BW1.data[i] != 0) || (BW2.data[i] != 0))
			indiceU += 1;
	}

	return(indiceI / indiceU);
}

IMAGERGB comparaisonImage(IMAGE BW1, IMAGE BW2)
{
	IMAGERGB rgb = { 0,0,NULL,NULL };

	rgb = allocationImageRGB(BW1.Nblig,BW1.Nbcol);
...
{
	IMAGE agrandie = allocationImage(img.Nblig + 2, img.Nbcol + 2);
	IMAGE out = allocationImage(img.Nblig, img.Nbcol);

	double vMax = sqrt(2) * 255;

	// gestion des bords et des coins
	int pix;
	agrandie.pixel[0][0] = img.pixel[0][0];
	agrandie.pixel[0][agrandie.Nbcol - 1] = img.pixel[0][img.Nbcol - 1];
	for (pix = 1; pix < agrandie.Nbcol - 1; pix++) {
		agrandie.pixel[0][pix] = img.pixel[0][pix - 1];
		agrandie.pixel[agrandie.Nblig - 1][pix] = img.pixel[img.Nblig - 1][pix - 1];
	}
	agrandie.pixel[agrandie.Nblig - 1][0] = img.pixel[img.Nblig - 1][0];
	agrandie.pixel[agrandie.Nblig - 1][agrandie.Nbcol - 1] = img.pixel[img.Nblig - 1][img.Nbcol - 1];
	for (pix = 1; pix < agrandie.Nblig - 1; pix++) {
		agrandie.pixel[pix][0] = img.pixel[pix - 1][0];
		agrandie.pixel[pix][agrandie.Nbcol - 1] = img.pixel[pix - 1][img.Nbcol - 1];
	}

	// gestion du coeur
	for (int i = 0; i < img.Nblig; i++)
		for (int j = 0; j < img.Nbcol; j++) {
			agrandie.pixel[i + 1][j + 1] = img.pixel[i][j];
		}

	// passage des noyaux

	for (int i = 1; i < agrandie.Nblig - 1; i++)
		for (int j = 1; j < agrandie.Nbcol - 1; j++)
		{
			double vx = (double)agrandie.pixel[i + 1][j] - (double)agrandie.pixel[i - 1][j];
			double vy = (double)agrandie.pixel[i][j + 1] - (double)agrandie.pixel[i][j - 1];

			out.pixel[i - 1][j - 1] = (unsigned char)(sqrt((vx * vx) + (vy * vy)) * (1 / sqrt(2)));
		}

	liberationImage(&agrandie);
